...
type Directive struct {
	Directive string     `json:"directive"`   // 当前的命令，即每行首个关键字  http、set、index
	Line      int        `json:"line"`			 // 关键字所在行数
	Args      []string   `json:"args"`			// 关键字的参数  {...}   index.html
	File      string     `json:"file,omitempty"`	//
	Includes  []int      `json:"includes,omitempty"`	// 当前关键字导入， include ...
	Block     Directives `json:"block,omitempty"`		// 关键字所在 block，例如 http、server
	Comment   *string    `json:"comment,omitempty"`		// 当前关键字的注释
}
