...
func (p *parser) parse(parsing *Config, tokens <-chan NgxToken, ctx blockCtx, consume bool) (parsed Directives, err error) {
	...
	for t := range tokens {
		...
		if strings.HasPrefix(t.Value, "#") && !t.IsQuoted {
			//fmt.Println("comment, value ", t.Value)
		...
		// 在这个 for 循环中获取该 key 的所有参数
		// 当前 单词 不在引号后者不是特殊字符，则将 token 合并到一起
		for t.IsQuoted || (t.Value != "{" && t.Value != ";" && t.Value != "}") {

			if !strings.HasPrefix(t.Value, "#") || t.IsQuoted {
				// 在这里将 配置文件的每一行放在一起
				stmt.Args = append(stmt.Args, t.Value)
			} else if p.options.ParseComments { // 这部分的代码未执行
		...
		// 如果遇到更高级的用法，则不继续解析，例如： geo [$address] $variable { ... }
		if len(ctx) > 0 {
			if _, ok := mapBodies[ctx[len(ctx)-1]]; ok {
				fmt.Println("in ctx, show parsed: ", parsed)
		...
		if t.Value == "{" && !t.IsQuoted {
			...
			fmt.Println("进入 block：", inner, stmt)
		...
		// 在这里收集同一个 block 中的关键字
		//fmt.Println("parsed = append(parsed, stmt)", stmt.Directive, "当前 block: ", ctx)
		parsed = append(parsed, stmt)
